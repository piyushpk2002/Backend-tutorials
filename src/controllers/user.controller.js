...
const updateUserAvatar = asyncHandler(async(req, res) =>{
   ...
   const user = await User.findByIdAndUpdate(req.body._id,
    {
      $set:{
        avatar: avatar.url //remember avatar is an object so we just update its url
      }
    },
    {new: true}
   ).select("-password")
...
const updateUserCoverImage = asyncHandler(async(req, res) =>{
  ...
  const user = User.findByIdAndUpdate(req.body._id,
   {
     $set:{
       coverImage: coverImage.url //remember avatar is an object so we just update its url
     }
   },
   {new: true}
  ).select("-password")
